...
async def record_legacy(pool: asyncpg.Pool, rows: list[tuple[str, int, str]], start_block: int) -> int:
    """Seed the ledger with every (hotkey, block, raw_payload) committed before ``start_block``.

    Idempotent: a hotkey is recorded at most once. Returns the number of rows newly inserted.
    """
    legacy = [(hk, block, raw) for hk, block, raw in rows if block < start_block]
    if not legacy:
        return 0
    inserted = 0
