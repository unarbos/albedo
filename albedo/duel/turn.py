...
async def judge_generated_turn(
    ...
    *,
    ...
    # Strip injection, build the pairwise messages once, and score ALL judges via the
    # unified transport in a single call (sequential stream-gated Chutes -> batched
    # OpenRouter). A judge that neither provider resolves comes back as None -> parse-fail
    # sentinel, so one judge can't abort the turn (mislabelled as vllm_error).
    king_clean = strip_reply_injection(king_reply)
    chal_clean = strip_reply_injection(chal_reply)
